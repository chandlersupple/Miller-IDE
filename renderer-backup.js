...
To do:
1st Priority:
- Fix canvas size, rotation about CENTER (Referring to the interface)
- Organize the code

2nd Priority:
- Add undo, redo
- Add "Open File" prompt to workspace
- Add arrows selector
- Add way to enter in values into "Rotate"
- Create a visual for "Translate along ..."
- Increase density of grid-lines
- Mark the work-offset
- Create a web-friendly version for iPad
- Make nav-bar resizable
- Fix the scale of the workspace
*/
...
function plotPoints(program, canvas) {
    ...
    for (i = 0; i < points.length; i++) {
        points[i][0] = (canvas.offsetHeight / 2) * (points[i][0] - minY) / (maxY - minY);
        points[i][1] = (canvas.offsetHeight / 2) * (points[i][1] - minY) / (maxY - minY);
    }
    // Transform
    var rotate;
    ...
    for (r = 0; r < rotate; r++) {
        ...
        var avgX = xVals.reduce((a, b) => a + b, 0) / xVals.length;
        var avgY = yVals.reduce((a, b) => a + b, 0) / yVals.length;
        for (i = 0; i < points.length; i++) {
            points[i][0] -= avgX;
            points[i][1] -= avgY;
        }
...
function drawCanvas() {
    if (fileContents) {
        ...
        // Decrease the number of canvases if there are too many
            ...
            for (var k = 0; k < canvases.length; k++) {
                if (sbMarkers[sb] == 'M5' && k < canvases.length - 1) {
                    ...
                    if (sb != 0 && sb != subroutines.length - 1) {
                        tempSR.unshift(['( TOOLPATH: ' + sb + ', PATH: ' + k + ' )']);
